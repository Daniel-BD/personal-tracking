...
- **Always update the relevant AGENTS.md file(s)** when making changes to the codebase (new components, changed patterns, modified architecture, renamed files, etc.) so they continue to accurately reflect the actual code. Update the root file for app-wide changes, or the subdirectory file for localized changes. The sibling `CLAUDE.md` files are redirect markers only.
...
- **Styling**: Tailwind CSS v4 (Vite plugin, not PostCSS) — see `src/app/AGENTS.md` for styling guide
...
- **Animation**: Motion (`motion/react`) — see `src/shared/AGENTS.md` for animation utilities
...
- **External store**: Module singleton + `useSyncExternalStore` (not Context). Fine-grained selector hooks prevent re-renders. See `src/shared/AGENTS.md`.
- **Feature-owned derived indexes**: Build expensive cross-entity lookup maps in the owning feature on top of shared slice hooks rather than widening `shared/store`. Tracking now exports reusable entry/item/category index builders and hooks for Stats and Library-style consumers. See `src/features/tracking/AGENTS.md`.
...
- **App-owned notifications**: Shared store code emits typed semantic events; the app layer owns localized sync toasts and the sync status pill. Shared UI stays store-free. See `src/shared/AGENTS.md` and `src/app/AGENTS.md`.
- **Document-owned page scrolling**: The browser/document is the only page-level vertical scroller so the scrollbar stays at the viewport edge. The app shell keeps route content in a centered `max-w-4xl` inner wrapper above the fixed bottom nav; only intentional subregions (Home favorites/recent list, sheets, dropdowns) use internal scrolling.
...
See `src/app/AGENTS.md` for the full styling guide (color system, CSS classes, conventions). Read it when writing UI code.
