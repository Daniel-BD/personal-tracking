...
- **Swipe gestures**: `useSwipeGesture` hook (in `features/tracking/hooks/`, exported from `@/features/tracking` barrel) encapsulates touch-based swipe-left logic to reveal Edit (blue) and Delete (red) action buttons. Uses a 70px threshold. Returns `swipedEntryId`, `swipeOffset`, touch handlers, `resetSwipe`, and `handleRowTap`. Used by EntryList (Log page) and both ItemsTab/CategoriesTab (Library page).
