...
- **`SentimentPills.tsx`** — Compact positive/limit count pills (green `N+`, red `N−`). Takes `{ positive, limit }` number props. Used by `DaySentimentSummary` and `BalanceScoreMeter`.
- **`BalanceScoreMeter.tsx`** — Shared animated balance-score summary row + gradient meter used by Home and Stats so score typography, pill sizing, and bar height stay in sync across daily and weekly views.
