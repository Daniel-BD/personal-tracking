...
- **`ItemsTab.tsx`** — Item list across both activity and food types with sentiment accent dot, and a right-aligned type pill on the same row as the item title (via shared `TypePillTitle`), plus default-category sentiment pills. Row tap navigates to `/stats/item/:itemId`; inline favorite/edit/delete icon buttons are preserved. Add/edit BottomSheets include an in-sheet type picker (`TypeSegmentedPicker`) and use `CategoryPicker` for category assignment. Edit sheet includes "Merge into..." button.
- **`CategoriesTab.tsx`** — Category list across both activity and food types with sentiment dot and a right-aligned type pill on the same row as the category title (via shared `TypePillTitle`). Row tap navigates to `/stats/category/:categoryId`; inline edit/delete icon buttons are preserved. Add/edit BottomSheets include an in-sheet type picker (`TypeSegmentedPicker`). Edit sheet includes "Merge into..." button.
