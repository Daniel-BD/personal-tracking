...
- **`stats-engine.ts`** — Weekly food analytics, balance scores (based on category sentiment), actionable category rankings (top limit categories to reduce, lagging positive categories to increase). `SENTIMENT_COLORS` maps neutral sentiment to the shared neutral/primary blue token (`var(--color-neutral)`).
...
- **`FrequencyRanking.tsx`** — Ranked list of most-logged items or categories ordered by count, with SegmentedControl filters for time period (all time/7 days/30 days), type (all/activities/food), and view mode (items/categories). Item and category rows use shared sentiment color logic from `@/shared/lib/sentiment` (`getSentimentAccentColor`, `SENTIMENT_COLORS`). Rows are tappable and navigate to `/stats/item/:itemId` or `/stats/category/:categoryId`.
