...
- **`ItemDetailPage.tsx`** — Thin item detail view for `/stats/item/:itemId`, backed by `useItemDetailViewModel()`. Header actions use the shared warning-tone edit icon button style (`IconActionButton`) and route to `/library` for editing. Monthly and yearly sections now receive the item's prefiltered entry slice instead of refiltering the full store.
- **`CategoryDetailPage.tsx`** — Thin category detail view for `/stats/category/:categoryId`, backed by `useCategoryDetailViewModel()`. Header actions use the shared warning-tone edit icon button style (`IconActionButton`) and route to `/library` for editing. The `limit` sentiment "days since last logged" pill still uses the same calculation; monthly and yearly sections use the category's prefiltered entry slice.
