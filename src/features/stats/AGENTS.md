...
- **`BalanceOverview.tsx`** — Overall balance score visualization. The top weekly summary now reuses shared `BalanceScoreMeter` from Home (same animated number, sentiment pills, and 2.5-height gradient bar) while the trend and weekly breakdown charts remain in cards. Weekly charts render all weeks at full emphasis; there is no low-data fade state.
