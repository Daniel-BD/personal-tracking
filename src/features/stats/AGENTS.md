...
- Weekly line charts that can exceed viewport width should render inside `overflow-x-auto` wrappers with a computed `minWidth` (for example `weeks.length * 56`) so users can horizontally scroll instead of squashing labels and dots.
