...
export function useLogFilters() {
	...
	const categoryOptions = useMemo(
		() =>
			availableCategories.map((cat) => ({
				...
				name: cat.name,
				subtitle:
					typeFilter === 'all' ? (activityCategories.find((c) => c.id === cat.id) ? 'Activity' : 'Food') : undefined,
			})),
	...
	function handleTypeChange(newType: 'all' | 'activity' | 'food') {
		if (newType !== typeFilter) {
			if (newType === 'activity') {
				setSelectedCategories((prev) => prev.filter((id) => activityCategories.some((c) => c.id === id)));
				setSelectedItems((prev) => prev.filter((id) => activityItems.some((i) => i.id === id)));
			} else if (newType === 'food') {
				setSelectedCategories((prev) => prev.filter((id) => foodCategories.some((c) => c.id === id)));
				setSelectedItems((prev) => prev.filter((id) => foodItems.some((i) => i.id === id)));
			}
