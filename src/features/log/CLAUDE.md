...
- Entry rows can optionally show the food/activity pill right-aligned on the same row as the item title (via shared `TypePillTitle`) when `showType` is enabled by the page.
