<script lang="ts">
	import { entries, foodItems } from '$lib/store';
	...
	const foodEntries = $derived(filterEntriesByType($entries, 'food'));
	const thisMonthRange = $derived(getMonthRange());
	const thisMonthEntries = $derived(filterEntriesByDateRange(foodEntries, thisMonthRange));
</script>
...
<div class="space-y-4">
	...
	{#if showForm}
		<EntryForm type="food" onsave={handleSave} />
	{/if}

	<div class="bg-white rounded-lg shadow p-4">
		<div class="flex justify-between items-center mb-4">
			<h3 class="font-semibold text-gray-800">This Month</h3>
			<span class="text-sm text-gray-500">{thisMonthEntries.length} entries</span>
		</div>
	...
	{#if foodEntries.length > thisMonthEntries.length}
		<details class="bg-white rounded-lg shadow">
			<summary class="p-4 cursor-pointer font-semibold text-gray-800">
				Older entries ({foodEntries.length - thisMonthEntries.length})
			</summary>
			<div class="px-4 pb-4">
				<EntryList
					entries={foodEntries.filter(
